...
package net.pkhapps.vera.server.messaging.api;

import net.pkhapps.vera.server.messaging.domain.*;
import net.pkhapps.vera.server.util.Registration;

import java.util.Set;
import java.util.function.Consumer;

/// API for sending messages to terminals.
public interface ForSendingMessages {
    ...
    ///
    /// @param recipients a set of at least one [TerminalId] to send the message to
    /// @param priority   the message priority
    /// @param payload    the message payload
    /// @return an [OutgoingMessage]
    OutgoingMessage sendMessage(Set<TerminalId> recipients, MessagePriority priority, MessagePayload payload);
