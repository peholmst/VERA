...
inline void PaintFilledCircle(const RendererPtr &renderer, const int centerX, const int centerY, const int r, const uint32_t color)
{
    ...
    {
        int dx_limit = (int)std::sqrt(r * r - dy * dy);
...
inline void PaintLine(const RendererPtr &renderer, const float x1, const float y1, const float x2, const float y2, const float thickness, const uint32_t color)
{
    const Uint8 *c = (Uint8 *)&color;
